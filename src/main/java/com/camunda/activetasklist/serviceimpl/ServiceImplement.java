...
//	9. passing variable name get Task Id :getVaribl

	@Override
	public List<Task> passingVarNameGetActiveTaskID(String envirnment, String varName) {
		final List finalList = new ArrayList<>();
		Map<String, Object> getVariabl1e = null;
		if (envirnment.equalsIgnoreCase(camundaSAAS)) {
			try {
				tasks = camundaServiceUtility.getTaskSaasAuthentication();
			} catch (Exception e) {
				e.printStackTrace();
		...
		// Self-Managed :
		if (envirnment.equalsIgnoreCase(camundaLocal)) {
			try {
				tasks = camundaServiceUtility.getTaskSimpleAuthentication();
			} catch (Exception e) {
				e.printStackTrace();
			}
			for (Task getVar : tasks) {
				List<Variable> variables = getVar.getVariables();
				for (Variable getVariableName : variables) {
					String name = getVariableName.getName();
					if (name.equals(varName)) {
						finalList.add(getVar);
					}
				}
			}
		}
		// for elastic search :
		if (envirnment.equalsIgnoreCase(localelasticsearchenv)) {
			ResponseEntity<Map> responseEntity = restTemplate.exchange(selfmanagedzeeberecordUrls, HttpMethod.POST,
					null, Map.class);
			Map resBody = responseEntity.getBody();
			...
			for (Object itrHits : getHitsList) {
				...
				getVariabl1e = (Map<String, Object>) getvalueMapObj.get("variables");
				for (Map.Entry m : getVariabl1e.entrySet()) {
					String key = (String) m.getKey();
					if (key.equals(varName)) {
						finalList.add(getSource);
					}
				}
			}
		}
